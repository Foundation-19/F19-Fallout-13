...
/obj/structure/closet/crate/grave/tool_interact(obj/item/S, mob/living/carbon/user)
	if(user.a_intent != INTENT_HARM) //checks to attempt to dig the grave, must be done with combat mode off only.
		if(!opened)
	...
	else if((user.a_intent == INTENT_HARM) && opened) //checks to attempt to remove the grave entirely.
		if(istype(S,cutting_tool) && S.tool_behaviour == TOOL_SHOVEL)
