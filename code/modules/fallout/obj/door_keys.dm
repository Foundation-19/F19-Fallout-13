...
/obj/item/door_key/attack_self(mob/user as mob)
	if(!id)
		return
	src.name = input("Give the name of the tag.",,copytext_char(src.name,0, length(src.name) - 3)) + " key"
	return
...
/obj/item/door_key/attackby(obj/item/W, mob/user, params)
	if(istype(W,/obj/item/door_key))
		...
		if((!src.id && !K.id) || (src.id && K.id))
			return 0
		if(alert(user,"Do you want to make a copy of the key?",,"Yes","No") == "No")
			return 0
		if(src.id && !K.id)
			K.attach_id(src.id)
		else
...
/obj/item/lock/attackby(obj/item/W, mob/user, params)
	...
	if(istype(W,/obj/item/door_key))
		...
		if(id)
			if(id == K.id)
				to_chat(user, "<span class='notice'>you start [open ? "close" : "open"] lock.</span>")
				if(do_after(user, 15, target = loc))
					if(!open && jammed)
						to_chat(user, "<span class='userdanger'>The key turns tightly in the lock, it looks like someone tried to break it!</span>")
					toogle()
			else
				to_chat(user, "<span class='warning'>Invalid key!</span>")
		else
			if(K.id)
				attach_id(K.id)
	...
	if(istype(W,/obj/item/lockpick))
		if(jammed)
			to_chat(user, "<span class='warning'>This lock has jammed, now it can only be opened with a key..</span>")
			return

		if(world.time < spam_protect_time)
			to_chat(user, "<span class='warning'>You need a little time to concentrate.</span>")
			return
		...
		if(id && !open)
			to_chat(user, "<span class='warning'>You start to pick the lock.</span>")
			if(do_after(user, 50, target = loc))
				switch(rand(1,3))
					if(1)
						toogle()
						to_chat(user, "<span class='green'>You opened the lock!.</span>")
						return
					if(2)
						if(prob(jammed_chance))
							jammed = TRUE
							to_chat(user, "<span class='userdanger'>The lock is jammed!</span>")
						else
							to_chat(user, "<span class='warning'>You failed to pick the lock!</span>")
					if(3)
						to_chat(user, "<span class='warning'>[L.name] broke!</span>")
						qdel(L)
	. = ..()
...
/obj/item/lockpick
	name = "lockpick"
	desc = "An ordinary hair clip, but in skillful hands it can act as a lockpick."
	icon = 'icons/fallout/objects/keys.dmi'
	icon_state = "Hairpin"
	w_class = WEIGHT_CLASS_TINY
...
/obj/item/storage/bag/lockpicks
	name = "Box of Lockpicks"
	desc = "Small pre-war box with lockpicks."
