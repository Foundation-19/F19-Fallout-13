...
/datum/emote/living/scream/run_emote(mob/living/user, params) //I can't not port this shit, come on.
	...
	if(!user.is_muzzled() && !miming)
		...
		if(ismonkey(user))
			sound = 'fallout/code/modular_citadel/sound/voice/scream_monkey.ogg'
		if(istype(user, /mob/living/simple_animal/hostile/gorilla))
			sound = 'sound/creatures/gorilla.ogg'
		if(ishuman(user))
			user.adjustOxyLoss(5)
			sound = pick('sound/effects/male_scream.ogg', 'fallout/code/modular_citadel/sound/voice/scream_m1.ogg', 'fallout/code/modular_citadel/sound/voice/scream_m2.ogg')
			if(user.gender == FEMALE)
				sound = pick('sound/effects/female_scream.ogg', 'fallout/code/modular_citadel/sound/voice/scream_f1.ogg', 'fallout/code/modular_citadel/sound/voice/scream_f2.ogg')
		if(isalien(user))
...
/datum/emote/living/snap/run_emote(mob/living/user, params)
	...
	if(user.nextsoundemote >= world.time)
		return
	user.nextsoundemote = world.time + 7
	playsound(user, 'fallout/code/modular_citadel/sound/voice/snap.ogg', 50, 1, -1)
...
/datum/emote/living/snap2/run_emote(mob/living/user, params)
	...
	if(user.nextsoundemote >= world.time)
		return
	user.nextsoundemote = world.time + 7
	playsound(user, 'fallout/code/modular_citadel/sound/voice/snap2.ogg', 50, 1, -1)
...
/datum/emote/living/snap3/run_emote(mob/living/user, params)
	...
	if(user.nextsoundemote >= world.time)
		return
	user.nextsoundemote = world.time + 7
	playsound(user, 'fallout/code/modular_citadel/sound/voice/snap3.ogg', 50, 1, -1)
