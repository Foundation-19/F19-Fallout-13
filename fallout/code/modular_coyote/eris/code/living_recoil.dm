...
/mob/living/proc/update_cursor(obj/item/gun/G)
	...
	if(client)
		...
		var/icon/base = find_cursor_icon('fallout/code/modular_coyote/eris/icons/standard.dmi', offset)
		ASSERT(isicon(base))
...
/proc/make_cursor_icon(icon_file, offset)
	var/icon/base = icon('fallout/code/modular_coyote/eris/icons/96x96.dmi')
	var/icon/scaled = icon('fallout/code/modular_coyote/eris/icons/standard.dmi') //Default cursor, cut into pieces according to their direction
	base.Blend(scaled, ICON_OVERLAY, x = 32, y = 32)

	for(var/dir in list(NORTHEAST,NORTHWEST,SOUTHEAST,SOUTHWEST))
		var/icon/overlay = icon('fallout/code/modular_coyote/eris/icons/standard.dmi', "[dir]")
		var/pixel_y
		var/pixel_x
		if(dir & NORTH)
	...
	add_cursor_icon(base, 'fallout/code/modular_coyote/eris/icons/standard.dmi', offset)
	return base
