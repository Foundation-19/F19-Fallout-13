...
/obj/structure/obstacle
	...
	desc = "don't use"
	icon = 'fallout/icons/structures/barricades.dmi'
	anchored = TRUE
	density = TRUE
	obj_integrity = 150
	max_integrity = 150
...
/obj/structure/junk
	icon = 'fallout/icons/objects/furniture/junk.dmi'
	obj_integrity = 100
	max_integrity = 100
	anchored = 1
	density = 1
	...
	if(!istype(src, /obj/structure/barricade/wooden/planks) && !istype(src, /obj/structure/barricade/wooden/crude) && istype(I, /obj/item/stack/sheet/))
		if(isfloorturf(loc) || isplatingturf(loc))
			if(istype(I, /obj/item/stack/sheet/mineral/wood))
				...
				if(W.amount >= 3)
					var/list/walls = list(
						"Wooden Wall" = image(icon = 'fallout/icons/turfs/walls/wood.dmi', icon_state = "wood0"),
						"Interior Wall" = image(icon = 'fallout/icons/turfs/walls/interior.dmi', icon_state = "interior0"),
						"House Wall" = image(icon = 'fallout/icons/turfs/walls/house.dmi', icon_state = "house0")
					)
				...
				if(C.amount >= 3)
					var/list/tentwalls = list(
						"Tent Wall" = image(icon = 'fallout/icons/turfs/walls/tent.dmi', icon_state = "tent0"),
						"Tent Flaps" = image(icon = 'fallout/icons/structures/doors.dmi', icon_state = "tent")
					)
